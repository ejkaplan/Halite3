...
class GameMap:
    ...
    def position_in_direction(self, source, direction):
        """
        Finds the position in a particular direction within the game map
        :param source: the starting position
        :param direction: the direction to travel from source
        :return: a Position object representing the space in that direction
        """
        if type(direction) == str:
            direction = Direction.char_to_tuple(direction)
        new_pos = source.directional_offset(direction)
        return self.normalize(new_pos)
